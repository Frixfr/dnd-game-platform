// server/src/controllers/npcEffectsController.ts
...
export const npcEffectsController = {
  async getAll(req: Request, res: Response) {
    try {
      ...
      const data = await npcEffectsService.getAll(roomId, {
        npc_id,
        effect_id,
        source_type,
        with_details,
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      const result = await npcEffectsService.create(roomId, {
        npc_id,
        effect_id,
        source_type,
        source_id,
      ...
      const io = getIO();
      io.to(`room:${roomId}`).emit("npc_effect:created", result);
  ...
  async delete(req: Request, res: Response) {
    try {
      ...
      await npcEffectsService.delete(roomId, npc_id, effect_id);
      const io = getIO();
      io.to(`room:${roomId}`).emit("npc_effect:deleted", { npc_id, effect_id });
