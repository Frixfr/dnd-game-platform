// server/src/controllers/playerAbilitiesController.ts
...
export const playerAbilitiesController = {
  async getAll(req: Request, res: Response) {
    try {
      ...
      const data = await playerAbilitiesService.getAll(roomId, {
        player_id,
        ability_id,
        is_active,
        with_details,
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      const io = getIO();
      io.to(`room:${roomId}`).emit("player_ability:created", result);
  ...
  async delete(req: Request, res: Response) {
    try {
      ...
      await playerAbilitiesService.delete(roomId, player_id, ability_id);
      const io = getIO();
      io.to(`room:${roomId}`).emit("player_ability:deleted", {
        player_id,
        ability_id,
      });
