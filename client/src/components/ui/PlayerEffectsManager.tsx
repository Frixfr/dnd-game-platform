  ...
  const renderAddEffects = () => {
    ...
    return (
      <div className="space-y-4">
        <input type="text" placeholder="🔍 Поиск эффектов..." value={effectSearch} onChange={e => setEffectSearch(e.target.value)} className="form-input w-full" />
        ...
        <div className="pt-2 flex flex-col sm:flex-row justify-between items-center gap-2 border-t border-border-color">
          ...
          <button onClick={handleAddEffects} disabled={selectedEffects.length === 0 || loading} className="w-full sm:w-auto px-4 py-2 btn-primary">Добавить выбранные</button>
        </div>
        <button onClick={() => setEffectsSubTab('list')} className="mt-2 text-sm text-text-secondary hover:text-text-primary transition-colors">← Назад к списку</button>
      </div>
    );
  };
      ...
      {effectsSubTab === 'list' ? (
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => { setEffectsSubTab('add'); setSelectedEffects([]); }} className="px-3 py-1 btn-secondary text-sm">➕ Добавить эффект</button>
          </div>
